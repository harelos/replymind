...
async function metrics(u: URL, env: Env) {
  ...
  const rows = await env.DB.prepare(
    `SELECT
      ...
      COUNT(DISTINCT CASE WHEN event_type IN('step_view','step_viewed') AND step_id LIKE '%advertorial%' THEN session_id END) advertorial,
      COUNT(DISTINCT CASE WHEN event_type IN('step_view','step_viewed') AND step_id LIKE '%sales%' THEN session_id END) sales,
      ...
      COUNT(DISTINCT CASE WHEN event_type IN('purchase','purchase_confirmed') AND provider IS NOT NULL THEN external_id END) purchases,
      COUNT(DISTINCT CASE WHEN event_type IN('refund','refund_confirmed') AND provider IS NOT NULL THEN external_id END) refunds,
      COALESCE(SUM(CASE WHEN event_type IN('purchase','purchase_confirmed','subscription_started','subscription_renewed') AND provider IS NOT NULL THEN amount_minor WHEN event_type IN('refund','refund_confirmed') AND provider IS NOT NULL THEN -amount_minor ELSE 0 END),0) revenue_minor,
